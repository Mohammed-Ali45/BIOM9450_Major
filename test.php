...
if (isset($_SESSION['email'])) {
    ...
    if(!extension_loaded('pdo')) {
      ...
      $dsn = 'Driver= {Microsoft Access Driver (*.mdb, *.accdb)};DBQ=C:\Users\User\Downloads\UNSW\Current\BIOM9450\Mutation.accdb';
    ...
    <body>
        ...
        //                         )";
        // $data = odbc_exec( $conn, "EXEC Query6" );
        // // $data = odbc_exec( $conn, "EXEC Query7" );
        // //$table = odbc_result($data , 'Repeat');
        ...
        // $all_rows[] = $row;
        // }
        // echo print_r($all_rows);

        $stmt = $conn->prepare("EXEC Query6");
        $stmt->execute();
        ...
        // echo odbc_num_rows($patient_mutations_table);
        // while(odbc_fetch_row($patient_mutations_table))
        //     {
        //     echo odbc_result($patient_mutations_table, 1);
        //     }
        //$patient_mutations_count_row = "SELECT COUNT(*) AS patient_mutations_count FROM ($patient_mutations) AS test";

        //$test = odbc_exec($conn, $patient_mutations_count_row);
        //$test2 = odbc_result($test, 'patient_mutations_count');
